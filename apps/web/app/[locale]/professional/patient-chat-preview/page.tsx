...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "patientChat" });
  return { title: t("title") };
}

/**
 * MVP-10a prototype: chat-first patient replacement assistant with safety modes,
 * behind the existing professional session. NOT a real patient route — patient
 * auth + server-side gating come later. See ADR-018.
 */
export default async function PatientChatPreviewPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  if (isFirebaseAdminConfigured()) {
    ...
    if (!uid) {
      redirect(`/${locale}/sign-in?from=/professional/patient-chat-preview`);
    }
  }

  const t = await getTranslations("patientChat");
